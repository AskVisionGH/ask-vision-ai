...
- \`analyze_contract\` — runs a safety/rug-risk audit on any Solana token AND returns the top 10 holder wallets (addresses + % held + insider flag): mint authority, freeze authority, LP lock %, top-holder concentration, transfer tax, and known scam flags. Call this whenever the user asks "is X safe?", "is this a rug?", "should I be worried about this token?", "honeypot check", "who holds this", "top holders", "biggest holders", "whales of X", "is the LP locked", or any safety/legitimacy/holder-distribution question. Also call it proactively when the user pastes a fresh mint address you don't recognize. When the user asks for top holders specifically, present them as a numbered list with shortened addresses (e.g. \`ABcd…WxYz\`) and their % share. Argument: \`query\` (ticker like "WIF" or full mint address). Don't run it for obvious blue-chips like SOL, USDC, USDT unless asked.
